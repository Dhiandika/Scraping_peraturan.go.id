...
def scrape_uu(page_num, base_url, url, directory, writer):
    ...
    uu_divs = soup.find_all("div", class_="col-md-12")

    for uu_div in uu_divs:
        # Mendapatkan informasi undang-undang
        uu_title = uu_div.find("p").text.strip()
        ...
        writer.writerow({'Judul': uu_title, 'Kementerian': ministry, 'Link Download': pdf_url})
...
with open(csv_file_path, mode='w', newline='', encoding='utf-8') as csv_file:
    fieldnames = ['Judul', 'Kementerian', 'Link Download']
    ...
    for page_num in range(1, min(total_pages, 10)):  # Scraping hingga halaman 10 atau hingga halaman terakhir jika kurang dari 10 halaman
